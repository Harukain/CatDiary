...
`pnpm legal:audit` 读取 [法律文档源文件](./legal/README.md)，检查隐私政策和用户协议是否包含版本、生效日期、账号注销/删除渠道、权限用途、第三方处理者、联系方式等发布必要内容，并列出仍待产品所有者确认的占位项。`pnpm legal:gate` 是 Preview/Production 发布前门禁：最终法律文档源文件不得再包含占位符、草稿声明或疑似 Secret，且必须通过结构检查。CI 只运行 `pnpm test:legal-docs` 验证门禁逻辑，不替代最终法律/合规确认。
...
Development Build 真机调试前可执行平台预检：Android 使用 `pnpm android:preflight -- --fix --launch` 检查并配置 USB reverse；若 3000 或 8081 被其它项目占用，可改用 `ANDROID_API_PORT` 与 `ANDROID_METRO_PORT` 指向备用端口。预检通过后可执行 `pnpm android:smoke` 做轻量真机冒烟检查，确认 Development Build 已加载当前 bundle 且观察窗口内无明显 Android/RN 启动崩溃；需要沉淀证据时使用 `pnpm android:smoke -- --evidence-file docs/device-acceptance/<date>-android-smoke.json`，再用 `pnpm acceptance:android-smoke-evidence -- --file <device-evidence.json> --smoke-file <android-smoke.json>` 合并到本轮验收草稿。该合并只证明 Android 设备信息、App 版本、设备预检和启动崩溃检查，不等同于完整 MVP 真机回归。iPhone 使用 `EXPO_PUBLIC_API_URL='http://开发机局域网IP:3000/api/v1' IOS_METRO_URL='http://开发机局域网IP:8081' pnpm ios:preflight` 检查 Xcode 命令行工具、已信任设备、局域网 API 和 Metro；需要沉淀证据时使用 `pnpm ios:preflight -- --screen <size> --evidence-file docs/device-acceptance/<date>-ios-preflight.json`，再用 `pnpm acceptance:ios-preflight-evidence -- --file <device-evidence.json> --preflight-file <ios-preflight.json>` 合并 iOS 设备信息、App 版本和预检状态。iPhone 真机不能使用 `127.0.0.1`、`localhost` 或 `10.0.2.2` 访问开发机服务。
...
Preview/Production 缺少任一法律文档 URL 会在原生构建前失败。登录页会在用户获取验证码前提供两个可点击入口，“我的－协议与隐私”也会持续提供入口和数据导出/删除说明。URL 对应页面必须可在未登录状态访问，内容变更需保留版本与生效日期；不得先用空白页通过门禁再替换。正式正文应从 `docs/legal/PRIVACY_POLICY_DRAFT.md` 和 `docs/legal/USER_AGREEMENT_DRAFT.md` 衍生，补齐运营主体、联系邮箱、地域和处理者后通过 `pnpm legal:gate`。
...
iOS App 级 Privacy Manifest 必须通过 `pnpm test:privacy-manifest` 的真实 Prebuild 验证；App Store/Google Play 数据问卷以 [商店隐私披露基线](./STORE_PRIVACY_DISCLOSURE.md) 为工程事实来源。该基线不替代正式隐私政策，运营主体、联系邮箱、URL、存储地域和第三方处理者仍需产品所有者确认；法律正文源文件和发布前门禁见 [法律文档源文件](./legal/README.md)。
